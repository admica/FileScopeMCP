# Requirements Archive: Milestone v1.6 Symbol-Level Intelligence

**Status:** ✅ SHIPPED 2026-04-23
**Archived:** 2026-04-24 at milestone close
...
- All 30 REQ-IDs functionally satisfied and verified — see `v1.6-MILESTONE-AUDIT.md` for 3-source cross-reference (plan claim → verification → test).
- Phase 34 has formal VERIFICATION.md (14/14 truths verified at 2026-04-23T22:42Z). Phases 33 + 35 have retroactive VERIFICATION.md generated at milestone close.
- PERF-02 soft threshold met: scan wall-time +13.75% self / +9.64% medium-repo vs baseline — under 15% budget, well under 25% hard-fail.
- No deletion-tracking on `list_changed_since` (CHG-05) — intentionally deferred; tombstone table deferred with it.
- JSX components classified as `function` kind (SYM-07) rather than a new `component` kind — no heuristic needed.
...
- [x] **CHG-01**: New MCP tool `list_changed_since(since, maxItems?)` accepting ISO-8601 timestamp or 7+ char git commit SHA
- [x] **CHG-02**: Returns `[{path, mtime}]` — paths whose DB `mtime` is greater than the resolved `since` value
- [x] **CHG-03**: Git-SHA mode resolves via `git diff --name-only <sha> HEAD`, filtered to paths present in the DB
- [x] **CHG-04**: Error codes: `NOT_INITIALIZED`, `INVALID_SINCE` (unparseable timestamp/SHA), `NOT_GIT_REPO` (SHA mode without `.git`)
- [x] **CHG-05**: No deletion tracking — only returns files that currently exist in DB; deletion tombstones deferred to future milestone

### Watcher Integration

- [x] **WTC-01**: FileWatcher re-extracts symbols on file change using the same throttled single-pass AST walk as edges — no separate symbol watcher
- [x] **WTC-02**: FileWatcher on unlink invokes `deleteSymbolsForFile(path)` alongside existing edge cleanup
- [x] **WTC-03**: Symbols treated as stale under the same `mtime`-based staleness model as edges — no separate per-symbol freshness column

### Performance Budget

- [x] **PERF-01**: Baseline captured before Phase 33: FileScopeMCP self-scan wall time and `medium-repo` fixture scan wall time
- [x] **PERF-02**: Scan wall time MUST NOT regress more than 15% from baseline at end of milestone; hard fail at 25% regression — phases revisited before merge
...
|--------|-------------|-------|--------|
| SYM-01 | Top-level symbol extraction (TS/JS) | 33 | Satisfied |
| SYM-02 | Single-pass AST walk | 33 | Satisfied |
| SYM-03 | `symbols` table + migration | 33 | Satisfied |
| SYM-04 | Repository functions | 33 | Satisfied |
| SYM-05 | Migration-time bulk extraction | 33 | Satisfied |
| SYM-06 | `npm run inspect-symbols` CLI | 33 | Satisfied |
| SYM-07 | JSX components as function kind | 33 | Satisfied |
| SYM-08 | Re-exports not populated | 33 | Satisfied |
| IMP-01 | Imported-name + import-line extraction | 33 | Satisfied |
| IMP-02 | Namespace import as `*` placeholder | 33 | Satisfied |
| IMP-03 | Additive schema for imports | 33 | Satisfied |
| FIND-01 | `find_symbol` MCP tool | 34 | Satisfied |
| FIND-02 | Exact + prefix, case-sensitive | 34 | Satisfied |
| FIND-03 | `exportedOnly` default true | 34 | Satisfied |
| FIND-04 | Standardized `{items, total}` envelope | 34 | Satisfied |
| FIND-05 | Error codes | 34 | Satisfied |
| SUM-01 | `exports[]` on `get_file_summary` | 34 | Satisfied |
| SUM-02 | Rich `dependents[]` with importedNames | 34 | Satisfied |
| SUM-03 | Additive response schema | 34 | Satisfied |
| SUM-04 | Empty arrays when no symbols | 34 | Satisfied |
| CHG-01 | `list_changed_since` MCP tool | 35 | Satisfied |
| CHG-02 | `{path, mtime}` response | 35 | Satisfied |
| CHG-03 | Git-SHA mode | 35 | Satisfied |
| CHG-04 | Error codes | 35 | Satisfied |
| CHG-05 | No deletion tracking | 35 | Satisfied |
| WTC-01 | Watcher single-pass symbol re-extract | 35 | Satisfied |
| WTC-02 | Watcher unlink cleanup | 35 | Satisfied |
| WTC-03 | Reuse mtime staleness model | 35 | Satisfied |
| PERF-01 | Baseline capture | 33 | Satisfied |
| PERF-02 | Wall-time regression budget | 35 | Satisfied |
