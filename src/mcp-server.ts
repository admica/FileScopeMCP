...
// Generate Mermaid diagram from file tree
function generateMermaidDiagram(
  rootNode: FileNode, 
  diagramType: string = "dependency", 
  minImportance: number = 0,
  maxNodes: number = 50,
  focusPath?: string
): string {
  
  // Start with the diagram header based on type
  let diagram = '';
  
  if (diagramType === "dependency" || diagramType === "hybrid") {
    diagram = 'graph TD;\n';
  } else if (diagramType === "directory") {
    diagram = 'graph TD;\n';
  } else {
    return `Error: Unknown diagram type '${diagramType}'`;
  }
  
  // Get all file nodes as a flat array
  const allFiles = getAllFileNodes(rootNode);
  ...
  // Create a map of nodes already added
  const addedNodes = new Set<string>();
  
  // Track edges to avoid duplicates
  const addedEdges = new Set<string>();
  
  // Calculate importance color (0-10 scale, blue to red)
  function getImportanceColor(importance: number): string {
    const normalizedValue = Math.min(10, Math.max(0, importance)) / 10;
    // Go from blue (low importance) to red (high importance)
    return `#${Math.floor((1 - normalizedValue) * 255).toString(16).padStart(2, '0')}${Math.floor((1 - normalizedValue) * 255).toString(16).padStart(2, '0')}${Math.floor(normalizedValue * 255).toString(16).padStart(2, '0')}`;
  }
  
  // Generate node IDs that are valid in Mermaid (replace invalid chars)
  function getNodeId(filePath: string): string {
    const cleanPath = path.basename(filePath);
    return `node_${cleanPath.replace(/[^a-zA-Z0-9]/g, '_')}`;
  }
  ...
  for (const file of filteredFiles) {
    ...
    if (!addedNodes.has(nodeId)) {
      ...
      const label = `${path.basename(file.path)} [${importance.toFixed(1)}]`;
      
      diagram += `  ${nodeId}["${label}"]:::importance${Math.round(importance)};\n`;
      addedNodes.add(nodeId);
    }
  }
  
  // Add class definitions for importance levels
  diagram += '\n  %% Importance level styling\n';
  for (let i = 0; i <= 10; i++) {
    const color = getImportanceColor(i);
    diagram += `  classDef importance${i} fill:${color},stroke:#333,stroke-width:1px;\n`;
  }
  
  // Add edges for dependencies if needed
  if (diagramType === "dependency" || diagramType === "hybrid") {
    diagram += '\n  %% Dependencies\n';
    
    for (const file of filteredFiles) {
      const sourceId = getNodeId(file.path);
      
      // Add edges to dependencies if they're also in our filtered set
      if (file.dependencies && file.dependencies.length > 0) {
        for (const depPath of file.dependencies) {
          // Find the actual node object for this dependency
          const depNode = filteredFiles.find(f => {
            const normalizedPath = normalizePath(f.path);
            const normalizedDepPath = normalizePath(depPath);
            return normalizedPath === normalizedDepPath || 
                   normalizedPath.endsWith(normalizedDepPath) || 
                   normalizedDepPath.endsWith(normalizedPath);
          });
          
          if (depNode) {
            const targetId = getNodeId(depNode.path);
  ...
  // For directory type, show directory structure
  if (diagramType === "directory" || diagramType === "hybrid") {
    diagram += '\n  %% Directory structure\n';
    
    // Add directory nodes and relationships
    function processDirectories(node: FileNode, parentId?: string) {
      if (node.isDirectory && node.children) {
        const dirId = getNodeId(node.path);
        if (!addedNodes.has(dirId)) {
          diagram += `  ${dirId}["${path.basename(node.path)}"]:::directory;\n`;
          addedNodes.add(dirId);
        }
        
        if (parentId) {
          const edgeKey = `${parentId}->${dirId}`;
          if (!addedEdges.has(edgeKey)) {
            diagram += `  ${parentId} --> ${dirId};\n`;
            addedEdges.add(edgeKey);
          }
        }
        ...
        node.children.forEach(child => {
          if (child.isDirectory) {
            processDirectories(child, dirId);
          }
        });
      }
    }
    ...
    processDirectories(rootNode);
    
    // Add directory class
    diagram += '  classDef directory fill:#f9f,stroke:#333,stroke-width:1px;\n';
  }
  
  return diagram;
}

// Add tool to generate Mermaid diagram
server.tool("generate_mermaid_diagram", "Generate a Mermaid diagram of file relationships", {
  diagramType: z.enum(["dependency", "directory", "hybrid"]).describe("Type of diagram to generate"),
  minImportance: z.number().min(0).max(10).optional().describe("Minimum importance score for inclusion (0-10)"),
  maxNodes: z.number().positive().optional().describe("Maximum number of nodes to include"),
  includeHtml: z.boolean().optional().describe("Whether to return HTML with embedded rendering"),
  focusPath: z.string().optional().describe("Path to focus the diagram around")
}, async (params: { 
  diagramType: "dependency" | "directory" | "hybrid",
  ...
  console.error('Generating Mermaid diagram with params:', params);
  
  try {
    // Generate the Mermaid diagram
    const mermaidSyntax = generateMermaidDiagram(
      fileTree!,
      params.diagramType,
    ...
    // Return as plain text or HTML with rendering
...
<head>
  ...
  <title>File Relationship Diagram</title>
...
<body>
  <h1>File Relationship Diagram</h1>
