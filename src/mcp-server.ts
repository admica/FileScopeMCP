...
export function registerTools(server: McpServer, coordinator: ServerCoordinator): void {
  ...
  server.registerTool("find_callers", {
    ...
    description: [
      ...
      "`maxItems` defaults to 50, clamped to [1, 500].",
      "Response: `{items: [{path, name, kind, startLine, confidence}], total, truncated?: true, unresolvedCount, warning?: string}`.",
      ...
      "`warning` is present only when the symbol IS indexed but has zero caller edges. This means either the symbol genuinely has no callers, OR the static-analysis call-graph missed them (known gap for some patterns, e.g., React hook export forms). When you see `warning`, fall back to grep to verify before treating the result as authoritative.",
      "Self-calls (recursive functions) are excluded from results.",
      "Returns `NOT_INITIALIZED` if the server hasn't been set up. Zero matches for an unindexed name returns `{items: [], total: 0, unresolvedCount: 0}` (no warning); zero matches for an indexed name returns the same plus a `warning` field — never an error.",
  ...
  server.registerTool("find_callees", {
    ...
    description: [
      ...
      "`warning` is present only when the symbol IS indexed but has zero callee edges. This means either the symbol genuinely calls nothing, OR the static-analysis call-graph missed them (known gap for some patterns, e.g., dynamic dispatch, JSX-embedded calls). When you see `warning`, fall back to grep to verify before treating the result as authoritative.",
      "Self-calls (recursive functions) are excluded from results.",
      "Returns `NOT_INITIALIZED` if the server hasn't been set up. Zero matches for an unindexed name returns `{items: [], total: 0, unresolvedCount: 0}` (no warning); zero matches for an indexed name returns the same plus a `warning` field — never an error.",
